...
class AsignacionesService {
  ...
  static const API = 'http://3.128.29.238:8001/api/asignacion/'; //AWS
  static const headers = {
      'Content-Type': 'application/json'
    };
  // Recibe Listado GET
  Future<APIResponse<List<AsignacionForListing>>> getAsignacionesList() {
    print(API+'lista');
    return http.get(API + 'lista', headers: headers).then((data) {
      if (data.statusCode == 200) {
        final jsonData = json.decode(data.body);
        final asignacion = <AsignacionForListing>[];
        for (var item in jsonData) {
          asignacion.add(AsignacionForListing.fromJson(item));
        }
        return APIResponse<List<AsignacionForListing>>(data: asignacion);
      }
      return APIResponse<List<AsignacionForListing>>(error: true, errorMessage: 'Ha ocurrido un error');
    })
    .catchError((_) => APIResponse<List<AsignacionForListing>>(error: true, errorMessage: 'Ha ocurrido un error'));
  }
